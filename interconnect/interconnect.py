...
class Interconnect():
    '''
    *** currently minimum TSV array size is 3x3 ***
    ...
    '''
    def __init__(self, B, wire_spacing=0.15e-6, wire_width=0.15e-6,
                 wire_length=1e-3, metal_layer=5, segments=1,
                 Driver=Driver.predefined(), TSVs=False, TSV_radius=1e-6,
                 TSV_pitch=4e-6, TSV_length=50e-6, KOZ=8e-6,
                 n=None, m=None, ground_ring=False,
                 Driver_out=Driver.predefined()):
        if TSVs:
            self.is_3D = True
            if n is None:
                n = round(math.sqrt(B))
    ...
    def power(self, data_inst=None, mux=None, f=1e9):
        ...
        if data_inst is None:
            C = self.C_2D + self.C_3D if self.is_3D is True else np.copy(
                self.C_2D)
            return met.mean_power2(C, self.Driver.V_dd, f,
                                   self.Driver_out.C_in)  \
    ...
    def prop_delay(self, wc_switching=None, verbose=False):
        ...
        if self.is_3D is True:
            t_3d = met.delay(self._C_3D_g, self.R_3D, self.Driver.t_0(),
                             self.Driver_out.C_in, self.Driver.R_on(
