...
class DataStream:
    ...
    TWO POSSIBLE INSTANTIATIONS:
       # 1 DataStream.data_stream( ...)
       # 2 DataStream.data_stream.from_stoch( ...)
    ...
    def __init__(self, samples, B=8, fit_to_B=False, is_signed=False, id=None):
        ...
        if np.max(samples) > v_range[1]:
            if fit_to_B:
                ...
                raise ValueError(
                    "Values range %d to %d to large for %d bits"
    ...
    def __getitem__(self, idx):
        ...
        return DataStream(
            B=self.B, samples=samples, is_signed=self.is_signed, fit_to_B=self.fitted
        )  # NEW ID!
    ...
    def __eq__(self, ds2):
        return (
            (self.B == ds2.B)
            and np.array_equal(self.samples, ds2.samples)
    ...
    def append(self, ds2):
        ...
        self.__init__(
            samples, self.B, fit_to_B=self.fitted, is_signed=self.is_signed, id=self.id
        )
    ...
    def check_B(self, ds2, ref=None):
        ...
        if ref != self.B or ref != ds2.B:
            raise ValueError(
                "Data streams %s and %s have different bitwidths" % (self.id, ds2.id)
            )
    ...
    def check_B_and_type(self, ds2):
        ...
        if self.is_signed != ds2.is_signed:
            raise ValueError(
                "Different signs for data streams %s and %s" % (self.id, ds2.id)
            )
    ...
    def check_comp(self, ds2):
        ...
        if len(self.samples) != len(ds2.samples):
            raise ValueError(
                "Data streams %s and %s have different lengths" % (self.id, ds2.id)
            )
    ...
    def k0_encoded(self, LSB_untouched=False):
        ...
        return DataStream(
            ...
            fit_to_B=self.fitted,
            id=(
                " KO-coded(%s), with LSB untouched is  (%s)" % (self.id, LSB_untouched)
            ),
        )  # NEW ID!

    def sign_mag_encoded(self, LSB_untouched=False):
        """
        Sign Magnitude encoder (XOR MSB + MSB!) (to do a plus 1)
        """
        samplesOLD = np.copy(self.as_unsigned.samples)  # bit vecs  as unsigned
        samples = samplesOLD ^ ((samplesOLD >> self.B - 1) * ((1 << self.B - 1) - 1))
        samples = samples + (samplesOLD >> self.B - 1)
        return DataStream(
            B=self.B,
            samples=samples,
            is_signed=False,
            fit_to_B=self.fitted,
            id=(" Sign-Magnitude-coded(%s)" % self.id),
        )  # NEW ID!
    ...
    def k1_encoded(self):
        ...
        return DataStream(
            ...
            samples=samples,
            fit_to_B=self.fitted,
            id=(" K1-coded(%s)" % self.id),
        )  # NEW ID!

    def k2_encoded(self):
        """
        k2 encoder (XOR 2nd MSB)
        """
        samples = np.copy(self.as_unsigned.samples)  # bit vecs  as unsigned
        samples_MSB2 = np.mod((samples >> (self.B - 2)), 2)  # extract 2nd MSB
        samples = samples ^ (samples_MSB2 * ((1 << self.B) - 1))  # XOR all with
        samples = samples + samples_MSB2 * (1 << (self.B - 2))  # undo XOR for 2nd MSB
        return DataStream(
            B=self.B,
            samples=samples,
            fit_to_B=self.fitted,
    ...
    def prob_invert(self):
        ...
        return DataStream(
            ...
            samples=samples,
            fit_to_B=self.fitted,
            id=("Prob. Invert (%s)" % self.id),
        )  # NEW ID!

    # LOW POWER RED. CODING
    def fixed_reli_encoder(self, p_subs=True):
        """
        Fixed encoder (extenstion of gray if p_subs = false), else inverted
        gray
        """
        samples = np.copy(self.as_unsigned.samples)  # bit vecs as unsigned
        samples = (samples << 1) ^ samples  # xor with shifted version
        if p_subs is True:
            samples = samples ^ ((1 << self.B) - 2)  # XOR 011...10 to ill XNOR
        return DataStream(
            B=self.B + 1,
            samples=samples,
            fit_to_B=self.fitted,
            id=("Redundancy fixed encoder (%s), p-doped is  (%s)" % (self.id, p_subs)),
        )  # NEW ID!

    def conf_reli_encoder(self, repair_sig, sa0=True):
        """
        conf encoder (modiffied inverter):
        repair_sig : line set to constant (to mask error), if bigger than
                     B then a cbi is used!
        sa0        : constant 0 on the line given by "repair_sig"?
        MODI:
          1. P-subs and the system should have the minimal capabilites
             (primary repair):
             "sa0= True" and "repair_sig" can be max. B (defect line)
          2. n-subs and the system should have the minimal capabilites
             (primary repair):
             "sa0= False" and "repair_sig" can be max. B (defect line)
          3. P-subs and the system should have repair and DSP LPC capabilities:
             Repair: See modus 1!; DSP "sa0=False" & "repair_sig=B-1"
          4. N-subs and the system should have repair and DSP LPC capabilities:
             Repair: See modus 2!; DSP "sa0=True" & "repair_sig=B-1"
          5. P-subs and the system should have repair and CBI LPC capabilities:
             Repair: See modus 1!; CBI: "repair_sig=B+1"
          6. N-subs and the system should have repair and CBI LPC capabilities:
             Repair: See modus 2!; CBI: "repair_sig=B+1"
        """
        if (repair_sig < 0) or (repair_sig > (self.B + 1)):
            raise ValueError(
                "Repair signature is %i. But has to be between 0 and %i"
                % (repair_sig, (self.B + 1))
            )
        if repair_sig == self.B + 1:
            return self.bus_invert()
        name = "Redundancy configurable encoder (%s), masked  %i, sa0: %s" % (
            self.id,
            repair_sig,
            sa0,
        )  # NEW ID!
        samples = np.copy(self.as_unsigned.samples)  # bit vecs as unsigned
        inv_line = (samples >> repair_sig) & 1  # isolate invert line
        inv_line = inv_line if sa0 is True else (inv_line ^ 1)  # inv sa1's
        invert_bits = inv_line * ((1 << (self.B + 1)) - 1)  # just a fanout
        samples = samples ^ invert_bits
        return DataStream(
            B=self.B + 1, samples=samples, fit_to_B=self.fitted, id=name
        )  # NEW ID!

    # Redundancy schemes with simple multiplexer
    def mux_reroute(self, repair_sig=0):
        """
        Signal reroute multiplexer scheme
        repair_sig: defines which signal is transmitted over the mux
        """
        if (repair_sig < 0) or (repair_sig > self.B):
            raise ValueError("Repair signature has to be between 0 and B")
        samples = np.copy(self.as_unsigned.samples)  # bit vecs as unsigned
        faulty_line = (samples >> repair_sig) & 1  # isolate faulty line
        samples = samples | (faulty_line << self.B)
        return DataStream(
            B=self.B + 1,
            samples=samples,
            fit_to_B=self.fitted,
            id=("Redundancy, reroute(%s). Broken line: (%i)" % (self.id, repair_sig)),
        )  # NEW ID!

    def mux_shift(self, repair_sig=0):
        """
        Signal shift multiplexer scheme
        repair_sig: defines which signal is transmitted over the mux
        """
        if (repair_sig < 0) or (repair_sig > self.B):
            raise ValueError("Repair signature has to be between 0 and B")
        samples1 = np.copy(self.as_unsigned.samples)  # upper mux inputs
        samples2 = samples1 << 1  # lower mux inputs
        samples_out = samples1 & 1  # over the first line always trans LSB
        for i in range(1, self.B):
            if i > repair_sig:
                samples_out = samples_out | (samples1 & (1 << i))
            else:
                samples_out = samples_out | (samples2 & (1 << i))
        samples_out = samples_out | (samples2 & (1 << self.B))  # MSB = in. MSB
        return DataStream(
            B=self.B + 1,
            samples=samples_out,
            fit_to_B=self.fitted,
            id=(
                "Redundancy, signal-shift(%s). Broken line: (%d)"
                % (self.id, repair_sig)
            ),
        )  # NEW ID!

    def coding_parity(self):
        """
        Adding parity bit as MSB (for error detection or defect-repair)
        """
        samples = np.copy(self.as_unsigned.samples)  # upper mux inputs
        for i in range(len(samples)):
            bin_vec = [
                int(np.binary_repr(samples[i], self.B)[j]) for j in range(self.B)
            ]
            parity = np.mod(np.sum(bin_vec), 2)
            samples[i] = samples[i] & (parity << self.B)  # MSB = parity
        return DataStream(
            B=self.B + 1,
            samples=samples,
            fit_to_B=self.fitted,
            id=("Parity check (%s)." % (self.id)),
        )  # NEW ID!
    ...
    def interleaving(self, other_ds, prob):
        ...
        for i in range(N):
            if random.random() < prob:  # change data stream
                ...
                other_ds = list(
                    list(range(0, curr_ds)) + list(range(curr_ds + 1, n_ds))
                )
        ...
        return DataStream(
            ...
            fit_to_B=self.fitted,
            is_signed=self.is_signed,
            id=("Interleaved with prob %.2f" % prob),
        )  # NEW ID!
    ...
    def as_unsigned(self):
        ...
        if self.is_signed is False:
            ...
            samples = [
                self.samples[x] - (self.samples[x] >> (self.B - 1)) * (1 << self.B)
    ...
    def invert(self):
        ...
        samples = [self.samples[i] ^ ones for i in range(len(self.samples))]
        return DataStream(
            B=self.B,
            samples=samples,
            is_signed=self.is_signed,
    ...
    def from_stoch(
        ...
        if uniform:
            ...
            if lognormal is None:
                samples = gaussian_data(N, B, log2_std, mu, ro, is_signed=is_signed)
            else:
                samples = lognormal_data(N, B, log2_std, mu, ro, is_signed=is_signed)
...
class DataStreamProb:
    ...
    def __init__(self, data_inst, mux=None):
        if isinstance(data_inst, (list, tuple)) is True:
            ...
            if mux.shape == (2 * len(data_inst), 2 * len(data_inst)) is False:
                raise IndexError(
                    "Mux has to be a quadratic matrix with a row "
                    + "column count of 2*len(data_inst)"
                )
            if (
                np.all(
                    [data_inst[x].B == data_inst[0].B for x in range(1, len(data_inst))]
                )
                is False
            ):
                raise ValueError("Data streams have different bit widths")
            self.B = data_inst[0].B
            toggle_mux = np.zeros((len(data_inst), len(data_inst), self.B))
            corr_mux = np.zeros((len(data_inst), len(data_inst), self.B, self.B))
            for d1 in range(len(data_inst)):
                toggle_mux[d1, d1, :] = data_inst[d1].toggle_prob_vec
                corr_mux[d1, d1, :, :] = np.copy(data_inst[d1].corr_switching_mat)
                for d2 in range(d1 + 1, len(data_inst)):
                    p1 = data_inst[d1].bit_prob_mat
                    p2 = data_inst[d2].bit_prob_mat
                    toggle_mux[d1, d2, :] = toggle_mux[d2, d1, :] = np.diag(p1) * (
                        1 - np.diag(p2)
                    ) + np.diag(p2) * (1 - np.diag(p1))
            ...
            self.corr_switching_mat = self.bit_prob_mat = np.zeros((self.B, self.B))
            ...
            for d1, d2 in itertools.product(range(len(data_inst)), repeat=2):
                ...
                self.corr_switching_mat = self.corr_switching_mat + var * np.array(
                    corr_mux[d1, d2, :, :]
                )  # += DOES NOT WORK!!!
    ...
    def from_probs(cls, self_switching, corr_switching, bit_prob, B=8):
        ...
        if isinstance(self_switching, (int, float)) is True:
            if np.size(corr_switching) == np.size(self_switching) == 1:
            ...
            if (
                (np.array(bit_prob).shape == np.array(corr_switching).shape == (B, B))
                and (np.size(self_switching) == B)
            ) is False:
                raise IndexError("All inputs have to be either int or arrays")
        return cls(DataProbContainer(self_switching, corr_switching, bit_prob, B))
